...
class TestLoadPrompt(unittest.TestCase):
    ...
    def test_successful_prompt_loading(self, mock_listdir, mock_exists):
        ...
        self.assertIn('This is the prompt content', result)
        self.assertIn('Some training data', result)
        self.assertIn('"word1": "[Word]"', result)  # Check for JSON format instructions
    ...
    def test_prompt_file_not_found(self, mock_listdir, mock_exists):
        ...
        result = load_prompt("Some training data")
        self.assertEqual(result, "Error: No prompt found. Please provide a 'prompt.txt' file in the 'data' folder.")
